...
export type FilterDropdownHandle = { reset: () => void };

type ParamKey = "popularity" | "materials" | "collections";
...
interface FilterDropdownProps {
  ...
  /** какой query-параметр обслуживает этот фильтр */
  paramKey: ParamKey;
  /** внешний сигнал (можно не использовать теперь, оставлен для совместимости) */
  resetSignal?: number;
}

const FilterDropdown = forwardRef<FilterDropdownHandle, FilterDropdownProps>(
  (
    {
      title,
      options,
    ...
    const writeToUrl = (arr: string[]) => {
      const next = new URLSearchParams(sp);
      if (arr.length) next.set(paramKey, arr.join(","));
    ...
    // отдадим наружу «тот самый» reset
    useImperativeHandle(ref, () => ({ reset: handleReset }), [sp]);

    // поддержка старого resetSignal (если где-то остался)
    useEffect(() => {
      if (resetSignal === undefined) return;
      // ничего не делаем, глобальный сброс теперь вызывает .reset()
    }, [resetSignal]);
